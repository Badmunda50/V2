...
class Call(PyTgCalls):
    ...
    async def trembl_boost_stream(self, chat_id: int, file_path, trembl_level, playing):
        assistant = await group_assistant(self, chat_id)
        base = os.path.basename(file_path)
        chatdir = os.path.join(os.getcwd(), "playback", "trembl", str(trembl_level))
        ...
        if not os.path.isfile(out):
            proc = await asyncio.create_subprocess_shell(
                cmd=(
                    f"ffmpeg -i {file_path} -af 'treble=g={trembl_level}' {out}"
                ),
        ...
        if str(db[chat_id][0]["file"]) == str(file_path):
            ...
            db[chat_id][0]["trembl_path"] = out
            db[chat_id][0]["trembl_level"] = trembl_level
    ...
    async def bass_boost_stream(self, chat_id: int, file_path, bass_level, playing):
        assistant = await group_assistant(self, chat_id)
        if not playing:
            raise AssistantErr("The 'playing' list is empty.")
