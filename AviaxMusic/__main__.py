...
import config
from AviaxMusic import LOGGER, app, userbot, Bad
...
async def init():
    ...
    await userbot.start()
    await Aviax.start()
    await Bad.start(bot_token=config.BOT_TOKEN)
    try:
    ...
    LOGGER("AviaxMusic").info(
        "\x41\x76\x69\x61\x78\x20\x4d\x75\x73\x69\x63\x20\x53\x74\x61\x72\x74\x65\x64\x20\x53\x75\x63\x63\x65\x73\x73\x66\x75\x6c\x6c\x79\x2e\x0a\x0a\x44\x6f\x6e\x27\x74\x20\x66\x6f\x72\x67\x65\x74\x2[...]
    )
    await idle()
    await app.stop()
    await userbot.stop()
